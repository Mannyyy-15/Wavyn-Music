    ...
    suspend fun identify(duration: Int, data: ByteArray): Track? {
        ...
        try {
            ...
            if (!response.isSuccessful) {
                 android.util.Log.e("WavynMusic", "API call failed: ${response.code()} ${response.message()}")
            }
            val track = response.body()?.track
            if (track != null) {
                android.util.Log.d("WavynMusic", "Track found: ${track.title} by ${track.subtitle}")
            } else {
                android.util.Log.w("WavynMusic", "API returned 200 but no track found (No Match)")
            }
            return track
        } catch (e: Exception) {
            android.util.Log.e("WavynMusic", "API network error: ${e.message}")
            e.printStackTrace()
            return null
        }
    }
